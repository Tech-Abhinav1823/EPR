...
const ExamSchedulePanel = () => {
  ...
  const getExamTypeIcon = (examType) => {
    switch (examType?.toLowerCase()) {
      case 'mid-semester': return 'BookOpen';
      case 'final': return 'Award';
      case 'practical': return 'Wrench';
      case 'quiz': return 'HelpCircle';
      case 'unit test': return 'FileText';
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-card-foreground">Exam Schedule</h3>
              <p className="text-sm text-muted-foreground">Mid-Semester Examinations</p>
            </div>
          </div>
        ...
        <div className="flex space-x-1 mb-4 bg-muted/50 rounded-lg p-1">
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors ${
              selectedExamType === 'upcoming' ?'bg-background text-foreground shadow-soft' :'text-muted-foreground hover:text-foreground'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors ${
              selectedExamType === 'completed'
                ? 'bg-background text-foreground shadow-soft' :'text-muted-foreground hover:text-foreground'
            }`}
          >
        ...
        <div className="space-y-4 max-h-80 overflow-y-auto">
          {currentExams?.map((exam) => (
            <div key={exam?.id} className="border border-border rounded-lg p-4 hover:bg-muted/30 transition-colors">
              {/* Exam Header */}
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start space-x-3 flex-1">
                  <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0">
                    <Icon name={getExamTypeIcon(exam?.examType)} size={18} className="text-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-semibold text-card-foreground line-clamp-1">
                      {exam?.subject}
                    </h4>
                    <p className="text-xs text-muted-foreground">{exam?.subjectCode} - {exam?.examType}</p>
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      />
                      {exam?.daysLeft && (
                        <span className={`text-xs font-medium ${getDaysLeftColor(exam?.daysLeft)}`}>
                ...
                {exam?.result && (
                  <div className="text-right">
                    <p className="text-sm font-bold text-success">{exam?.result}</p>
              ...
              <div className="ml-13 space-y-2">
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-card-foreground">{exam?.date}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Icon name="Clock" size={12} className="text-muted-foreground" />
                    <span className="text-muted-foreground">Time:</span>
                    <span className="font-medium text-card-foreground">{exam?.time}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Icon name="MapPin" size={12} className="text-muted-foreground" />
                    <span className="text-muted-foreground">Venue:</span>
                    <span className="font-medium text-card-foreground">{exam?.venue}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Icon name="User" size={12} className="text-muted-foreground" />
                    <span className="text-muted-foreground">Seat:</span>
                    <span className="font-medium text-card-foreground">{exam?.seat}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2 text-xs">
                  ...
                  <span className="font-medium text-card-foreground">{exam?.syllabus}</span>
                </div>
